...
def setup_args(**kargs):
    ...
    args = [
        get_nupack_exec_path(kargs['exec_name']), '-material',
        kargs['material'], '-sodium', kargs['sodium'], '-magnesium',
        kargs['magnesium'], '-dangles', kargs['dangles'], '-T', kargs['T']
    ]
    if kargs['multi']:
        args += ['-multi']
    if kargs['pseudo']:
...
def setup_nupack_input(**kargs):
    ...
    cmd_input = setup_cmd_input(kargs['multi'], kargs['sequences'],
                                kargs['ordering'], kargs.get('structure', ''))
...
def pfunc(sequences,
    ...
    # Perform call until it works (should we have a max # of tries before
    # quitting?)
    output, error = call_with_pipe(args, cmd_input)
    # can't figure out why, but occasionally NUPACK returns empty-handed.
    # Subsequent tries seem to work...
    while len(output) < 4:
        print(('Retrying pfunc: NUPACK failed with output ' + repr(output) +
              ' and error ' + repr(error) + " ."))
        output, error = call_with_pipe(args, cmd_input)
